...
from flask import Flask, redirect, url_for, session, json, g, flash, render_template, request, abort
...
app.config['SECRET_KEY'] = 'mydevkey'
...
def cached(timeout=5 * 60, key='cached/%s'): # ~200 req/s => ~600-800 req/s
    def decorator(f):
        @wraps(f)
